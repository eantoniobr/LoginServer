...
uses
  ScktComp, SysUtils, ClientPacket, Tools, ExtCtrls, Buffer, Clients, MyList, UWriteConsole;
...
begin
  ...
  begin
    if Client.GetPlayerUID = UID then
    begin
      Exit(Client);
    end;
  end;
...
procedure TAuthClient.HandleClientReceiveTicker(
...
var
  client : TClientPlayer;
begin
  for client in FClientlist do begin
    client.Send(Data);
  end;
end;

initialization
  AuthControlClient := TAuthClient.Create('127.0.0.1', 10111, 0);
finalization
  AuthControlClient.Free;
